...
type ExpensesContextType = {
  ...
  addExpense: (groupId: string, description: string, amount: number, payer: string) => void;
  getGroupById: (groupId: string) => Group | undefined;
  getExpensesByGroup: (groupId: string) => Expense[];
  computeBalances: (groupId: string) => Balances;
...
export const ExpensesProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const addGroup = useCallback((name: string, members: string[]) => {
    const trimmedMembers = members.map(m => m.trim()).filter(Boolean);
    const uniqueMembers = Array.from(new Set(trimmedMembers));
    const newGroup: Group = { id: generateId('grp'), name: name.trim() || 'Group', members: uniqueMembers };
    setGroups(prev => [newGroup, ...prev]);
  }, []);

  const addExpense = useCallback((groupId: string, description: string, amount: number, payer: string) => {
  ...
  const getExpensesByGroup = useCallback((groupId: string) => expenses.filter(e => e.groupId === groupId), [expenses]);

  const computeBalances = useCallback((groupId: string): Balances => {
    const group = groups.find(g => g.id === groupId);
    if (!group) return {};
    const groupExpenses = expenses.filter(e => e.groupId === groupId);
    const balances: Balances = Object.fromEntries(group.members.map(m => [m, 0]));
  ...
  const computeSettlements = useCallback((groupId: string): Settlement[] => {
    const group = groups.find(g => g.id === groupId);
    if (!group) return [];
